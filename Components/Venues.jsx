...
const Venues = ({ venue }) => {
  return (
    <section className="bg-black border-t-2  border-gray-900">
      <h1 className="text-3xl font-semibold text-white ml-3 mt-3 ">
        View our Venues :{" "}
      </h1>
      <div className="flex flex-wrap gap-5 m-5">
        {venue?.map((place) => (
          <div
            key={place.id}
            className="w-[300px] h-[350px] flex flex-col items-center bg-white rounded-xl"
          >
            ...
            <button className=" text-white p-4 bg-purple-950 rounded-full mt-2 ">
              Explore {place.Name}
            </button>
          </div>
        ))}
